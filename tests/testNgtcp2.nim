...
test "ptls_instantiation":
  var ctx: picotls.ptls_context_t
  ctx.random_bytes = ptls_openssl_random_bytes
  ctx.get_time = addr picotls.ptls_get_time
  ctx.key_exchanges =  cast[ptr ptr picotls.ptls_key_exchange_algorithm_t](addr ptls_openssl_key_exchanges[0])
  ctx.cipher_suites = cast[ptr ptr picotls.ptls_cipher_suite_t](ptls_openssl_cipher_suites[0])
  
  var tls: ptr picotls.ptls_t = ptls_client_new(addr ctx);
  check tls != nil
